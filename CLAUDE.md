...
- `src/config/site.ts` (`SITE` object) is the single source of company info (name, phone, email, address, default meta) — components like `Footer` read from it. `src/lib/constants.ts` holds standalone marketing copy (tagline/subline) but `Home.tsx` currently redefines the same strings locally instead of importing them (inconsistency to be aware of if editing copy).
