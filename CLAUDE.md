...
- Every page follows the same shape: `<SEO .../>` (from `src/lib/seo.tsx`, a `react-helmet-async` wrapper for title/description/canonical + Open Graph/Twitter card tags, image defaulting to `SITE.meta.image`) + `<PageHeader/>` + one or more `<Section/>` blocks (`src/components/layout/`). `Container` provides the centered max-width wrapper used inside `Section`/`PageHeader`/`Navbar`/`Footer`.
- This is a pure client-rendered SPA (no SSR/prerendering), so `SEO`'s per-page OG tags only exist after JS runs — link-preview bots (WhatsApp, LinkedIn, Slack, Facebook) generally don't execute JS and never see them. `index.html` therefore also carries static site-wide OG/Twitter tags as a fallback so shared links always get *some* correct preview (generic, not page-specific). If per-page-accurate share previews ever matter, that requires prerendering/SSR, not just more `Helmet` tags.
